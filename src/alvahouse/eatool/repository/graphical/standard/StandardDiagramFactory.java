...
/**
 * StandardDiagramFactory is a detail handler for deserialising
 * StandardDiagrams.
 * 
...
public class StandardDiagramFactory extends FactoryBase implements DiagramDetailFactory {
	...
	static {
		typeTranslation.put("alvahouse.eatool.gui.graphical.StraightConnectorStrategy",
				StraightConnectorStrategy.class.getName());
		typeTranslation.put("alvahouse.eatool.gui.graphical.QuadraticConnectorStrategy",
				QuadraticConnectorStrategy.class.getName());
		typeTranslation.put("alvahouse.eatool.gui.graphical.CubicConnectorStrategy",
				CubicConnectorStrategy.class.getName());
	}
	 ...
	 */
	// TODO tidy up shonky exception handling below.  Issue is that connectors / symbols can
	// refer to either a relationship or meta-relationship or an entity or meta - entity.
	// really factory should know what type of thing it's creating.
	public void startElement(String uri, String local, Attributes attrs) throws InputException {
		...
		if (local.equals("Symbol")) {
			...
			KeyedItem target = null;
			try {
				target = model.getEntity(objUUID);
			} catch (Exception e) {
			}
			...
			KeyedItem obj = null;
			try {
			obj = model.getRelationship(uuidObj);
			} catch(Exception e) {
			...
			if (obj == null) {
				try {
				obj = metaModel.getMetaRelationship(uuidObj);
				} catch (Exception e) {
			...
			if (obj == null) {
				throw new InputException(
						"Represents UUID does not correspond to a known Relationship or MetaRelationship");
			}
			...
			// Initialise the end positions - don't want to reset any handle positions by
			// doing it later.
