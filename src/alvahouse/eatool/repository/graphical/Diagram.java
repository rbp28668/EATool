...
public abstract class Diagram extends NamedRepositoryItem implements Versionable{
	...
	/**
	 * Removes the content of the diagram and sets any diagram attributes to their
	 * defaults.
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * Sets the colours and any other properties of the diagram to their default
	 * values (possibly determined by the diagram type).
	 */
	public abstract void resetPropertiesToDefaults();

	/**
	 * Signals that a repository item that would typically be attached to a node or
	 * symbol (i.e. an entity or meta-entity) has been removed from the model so
	 * that its corresponding graphical representation should be removed from the
	 * diagram as well.
	 * 
	...
	/**
	 * Signals that a repository item that would typically be attached to an arc or
	 * connector (i.e. a relationship or meta-relationship) has been removed from
	 * the model so that its corresponding graphical representation should be
	 * removed from the diagram as well.
	 * 
	...
	/**
	 * Takes 3 objects and sees if the arc corresponding to the connecting object
	 * has nodes that correspond to the start and finish objects. If so, then fine,
	 * otherwise the connector on the diagram is now invalid and must go. Used to
	 * signal changes to relationships in the model or meta-model so that diagrams
	 * can be updated.
	 * 
...
//
	...
	/**
	 * Called if event mapping scripts have been updated. Children of this may care
	 * to override this and provide custom behaviour such as saving the mapping.
	 * 
	 * @param scripts
	 * @throws Exception 
	 */
