...
import alvahouse.eatool.repository.dto.mapping.PropertyTranslationCollectionDto;
...
/**
 * This is a collection of PropertyTranslation that, as whole, can map, or
 * identify an entity.
 * 
 * @author rbp28668
 */
public abstract class PropertyTranslationCollection {
	...
	/**
	 * Creates a collection from its corresponding DTO.
	 * @param dto
	 */
	public PropertyTranslationCollection(PropertyTranslationCollectionDto dto)   {
		for (PropertyTranslationDto ptdto : dto.getProps()) {
			PropertyTranslation pt = new PropertyTranslation(ptdto);
			props.add(pt);
	...
	/**
	 * Looks up a property translation by the typename for that property where this
	 * typename is the name used to identify a property in a data import.
	 * 
	...
	/**
	 * given an entity, this works out a natural key string based on which property
	 * translations in the collection are marked as key properties
	 * 
	...
	String getKeyOf(Entity e) {
		...
		for (PropertyTranslation pt : getPropertyTranslations()) {
			if (pt.isKeyValue()) {
				Property p = e.getPropertyByMeta(pt.getMetaPropertyKey());
				String value = p.getValue().trim();
				if (value.length() == 0) {
					// throw new IllegalArgumentException("Key Property " + pt.getTypeName() + "
					// missing when importing XML");
	...
	protected void copyTo(PropertyTranslationCollectionDto dto){
		for (PropertyTranslation pt : props) {
			dto.getProps().add(pt.toDto());
		}
	}
